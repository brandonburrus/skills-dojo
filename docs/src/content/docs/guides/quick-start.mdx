...
4. **Run the evals**
   ...
   <Aside type="caution">
   Dojo requires an API key or SDK access for your configured model provider. The default provider is **Anthropic** (set `ANTHROPIC_API_KEY`). For OpenAI, set `OPENAI_API_KEY`. For Copilot, ensure you have an active subscription and are authenticated.
   </Aside>
   ...
   Dojo runs each eval against the configured model provider (Anthropic by default), shows pass/fail results, and writes a JSON report to `skills/sql-queries/evals/reports/<run-id>/report.json`.
