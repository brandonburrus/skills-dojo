...
When running the `with-distractors` variant for this eval, the selection pool includes the real skills plus all four decoys (three from the variant, one from the eval). If both define a decoy with the same name, the variant's version takes precedence.
