...
<CardGrid>
  ...
  <Card title="Effectiveness Evals" icon="star">
    Test whether a skill actually helps an agent produce correct output using LLM-as-a-judge scoring.
  </Card>
...
2. **Write evals** -- YAML files in a skill's `evals/` directory define prompts, variants, and expected outcomes. Selection evals test whether the agent picks the right skill; effectiveness evals test whether the skill helps the agent produce correct output.
3. **Run** -- Dojo presents the given eval prompt and available skills to an evaluator model in a sandbox environment. For selection evals, it tracks which skill the model selects. For effectiveness evals, it runs the agent with real tools and judges the output against your criteria.
4. **Assert** -- Selection evals check whether the agent's choice matches your assertion. Effectiveness evals check whether the judge's scores meet your pass thresholds.
...
<Aside type='tip' title='Multiple Model Providers'>
Skills Dojo supports multiple model providers: Anthropic (default), OpenAI, GitHub Copilot, and the Vercel AI SDK. Configure via `dojo.toml` or the `--model-provider` CLI flag.
</Aside>
