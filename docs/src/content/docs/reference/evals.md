...
| `timeout` | `number` | Yes | `30` | Default timeout in seconds for evals in this file. |
| `skills` | `"all"` \| `string[]` | Yes | `"all"` | Skills to register for evals. Use "all" or a list of skill names. |
| `run-mode` | `"all"` \| `"variants-only"` \| `"current-only"` | Yes | `"all"` | Default run mode for evals: "all", "variants-only", or "current-only". |
...
| `enabled` | `boolean` | Yes | `true` | Whether this eval is active. |
...
| `assert` | `string[]` \| `"none"` \| `"any"` | No | - | Expected skill selection. An array of skill names, "none" if no skill should load, or "any" to accept any selection. |
| `variants` | `"all"` \| `string[]` \| `Variant[]` | Yes | `"all"` | Variants to run: "all" uses file-level variants, or specify inline/by name. |
